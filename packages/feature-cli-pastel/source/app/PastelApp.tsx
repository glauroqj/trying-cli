...
// ---------------------------------------------------------------------------
// StepHeader with brief reveal animation
// ---------------------------------------------------------------------------
...
function TransitionScreen({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    if (frame >= SWEEP_FRAMES.length) { onDone(); return; }
...
export function PastelApp() {
  const { exit } = useApp();
  const welcome = getWelcomeContent();

  const [step, setStep] = useState<Step>('welcome');
  // Ref-based history avoids the anti-pattern of calling setStep inside setHistory's updater
  ...
  useEffect(() => {
    if (!showSub || step !== 'welcome') return;
  ...
  useInput((_input, key) => {
    if (step === 'welcome') { goTo('transition'); return; }
  ...
  if (step === 'transition') {
    return (
      <TransitionScreen
        onDone={() => { historyRef.current = []; setStep('mainAction'); }}
      />
    );
  }
  ...
  if (step === 'findStub') {
    return (
      <Box flexDirection="column">
        ...
        <Text>Para buscar no catálogo, use no terminal:</Text>
  ...
  if (step === 'listStub') {
    return (
      <Box flexDirection="column">
        ...
        <Text>Para listar, use:</Text>
        <Text color="yellow">  npx skills list</Text>
        <Text dimColor>Paths: ~/.cursor/skills/  ·  .agents/skills/</Text>
        <Box marginTop={1}>
          <SelectInput items={[BackItem]} onSelect={() => goBack()} />
        </Box>
      </Box>
      ...
      <Box flexDirection="column">
        ...
        <SelectInput
          ...
          onSelect={(item) => {
            ...
            recordAnswer('agentPick', selected?.detected
              ? `${selected.label} (detectado)`
      ...
      <Box flexDirection="column">
        ...
        <SelectInput
          items={[
            ...
            { label: 'Em um projeto', value: 'project' },
          ]}
          onSelect={(item) => {
            if (item.value === '__back__') { goBack(); return; }
        ...
        <SelectInput
          ...
          onSelect={(item) => {
            ...
            if (item.value.startsWith('__enter__:')) {
              void loadBrowseDirs(item.value.slice(10));
            }
          }}
        />
      </Box>
